...
router.get("/:mbid", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while fetching artist information" });
  }
});
...
router.get("/similar/:mbid", async (req, res) => {
  try {
    let mbid = req.params.mbid;
    ...
    console.error("Error getting similar artists:", error.message);
    res
      .status(500)
      .json({ error: "An error occurred while fetching similar artists" });
  }
});
